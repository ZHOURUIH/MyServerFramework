...
namespace FileUtility
{
...
#ifdef LINUX
	// 判断是否为目录
	bool isDirectory(const string& pszName)
	{
		struct stat S_stat;
		// 取得文件状态
	...
	void findFiles(const string& path, Vector<string>& files, const string* patterns, int patternCount, const bool recursive)
	{
		...
		while ((pDirent = readdir(pDir)) != nullptr)
		{
			...
			if (isDirectory(szTmpPath.str()))
			{
				...
				{
					//如果是文件夹则进行递归
					findFiles(szTmpPath.str(), files, patterns, patternCount, recursive);
				}
			}
			else
			{
	...
	bool deleteEmptyFolder(const string& path)
	{
		...
		while ((pDirent = readdir(pDir)) != nullptr)
		{
			...
			if (isDirectory(szTmpPath.str()))
			{
				...
				if (!deleteEmptyFolder(szTmpPath.str()))
				{
					isEmpty = false;
				}
			}
	...
	void deleteFolder(const string& path)
	{
		...
		while ((pDirent = readdir(pDir)) != nullptr)
		{
			...
			if (isDirectory(szTmpPath.str()))
			{
				deleteFolder(szTmpPath.str());
			}
			// 是文件,则删除文件
			else
			{
				remove(szTmpPath.str());
			}
		}
		closedir(pDir);
		rmdir(tempPath.c_str());
