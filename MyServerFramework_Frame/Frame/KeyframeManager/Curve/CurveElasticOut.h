...
class MICRO_LEGEND_FRAME_API CurveElasticOut : public AnimationCurve
{
	BASE(CurveElasticOut, AnimationCurve);
public:
	float evaluate(float time) const override
	{
		...
		const float s1 = period / TWO_PI_RADIAN * asin(1.0f / mOvershootOrAmplitude);
